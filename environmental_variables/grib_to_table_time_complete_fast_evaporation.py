...
for c in coordinates:
    ...
    while end_date < start_date:
        ...
        max_year[str(end_date.strftime("%Y"))][str(end_date.strftime("%m"))][str(end_date.strftime("%d"))] = float(min(day))
        min_year[str(end_date.strftime("%Y"))][str(end_date.strftime("%m"))][str(end_date.strftime("%d"))] = float(max(day))
        ...
        for dt in rrule.rrule(rrule.DAILY, dtstart = month_list[m], until=month_list[m+1]):
            if month_list[m] <= dt < month_list[m+1]:
                ...
                if len(day_list) == 0:
                    ...
                    print(total_list)
    print >> general_file, number + "_" + code + "_" + place + "_" + year + "\t" + collection + "\t" + str(numpy.mean(all_months)) + "\t" + str(min(max_month)) + "\t" + str(max(min_month)) + "\t" + str(numpy.std(all_months))
