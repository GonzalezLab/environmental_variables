...
for c in coordinates:
    ...
    while end_date < start_date:
        ...
        for d in day_file:
            line = '\t'.join(d.split())
            if grib_value == "1":
                day.append(abs(float(line.split("\t")[10])))
            elif grib_value == "2":
                day.append(abs(float(line.split("\t")[11])))
            elif grib_value == "3":
                day.append(abs(float(line.split("\t")[12])))
            elif grib_value == "4":
                day.append(abs(float(line.split("\t")[13])))
        ...
        min_year[str(end_date.strftime("%Y"))][str(end_date.strftime("%m"))][str(end_date.strftime("%d"))] = float(min(day))
